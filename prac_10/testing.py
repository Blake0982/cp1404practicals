...
def run_tests():
    ...
    assert test_car.fuel == 0, "default fuel is not 0"
    assert not test_car.fuel
    test_car = Car(fuel=10)
    assert test_car.fuel == 10, "fuel isn't added properly"
    assert not is_long_word("not")
...
def phrase_to_sentence(phrase):
    ...
    'It is an ex parrot.'
    >>> phrase_to_sentence('thats valid')
    'Thats valid.'
    """
